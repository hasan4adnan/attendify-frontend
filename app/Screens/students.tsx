    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <div className="w-full border-b border-[var(--border-primary)] bg-[var(--bg-secondary)] px-0 md:px-8 pt-8 pb-0 flex flex-col gap-0">
        <div className="flex flex-col items-center gap-2 px-6 md:px-0">
          ...
          <div className="flex gap-2 border-b border-[var(--border-primary)] justify-center">
            <button
              className={`px-4 py-2 text-base font-medium border-b-2 transition-all duration-200 focus:outline-none ${activeTab === 'list' ? 'border-[#0046FF] text-[#0046FF] bg-transparent' : 'border-transparent text-[var(--text-secondary)] hover:text-[#0046FF]'}`}
              onClick={() => setActiveTab('list')}
            >
              Öğrenci Listesi
            </button>
            <button
              className={`px-4 py-2 text-base font-medium border-b-2 transition-all duration-200 focus:outline-none ${activeTab === 'add' ? 'border-[#0046FF] text-[#0046FF] bg-transparent' : 'border-transparent text-[var(--text-secondary)] hover:text-[#0046FF]'}`}
      ...
      <div className="flex-1 w-full max-w-4xl mx-auto px-4 py-8">
        {activeTab === 'add' && (
          <form onSubmit={handleAddStudent} className="bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-2xl shadow p-8 max-w-2xl mx-auto animate-fadeIn">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-3 rounded-lg border transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#0046FF]/30"
                  style={{ backgroundColor: 'var(--bg-tertiary)', borderColor: focused === 'firstName' ? '#0046FF' : 'var(--border-primary)', color: 'var(--text-primary)' }}
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-3 rounded-lg border transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#0046FF]/30"
                  style={{ backgroundColor: 'var(--bg-tertiary)', borderColor: focused === 'lastName' ? '#0046FF' : 'var(--border-primary)', color: 'var(--text-primary)' }}
                  placeholder="Öğrenci soyadı"
                />
              </div>
              <div className="space-y-2">
                <label htmlFor="studentNumber" className="block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>
                  Öğrenci No
                </label>
                <input
                  id="studentNumber"
                  ...
                  required
                  className="w-full px-4 py-3 rounded-lg border transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#0046FF]/30"
                  style={{ backgroundColor: 'var(--bg-tertiary)', borderColor: focused === 'studentNumber' ? '#0046FF' : 'var(--border-primary)', color: 'var(--text-primary)' }}
            ...
            <button
              type="submit"
              className="mt-8 w-full py-3 px-6 bg-gradient-to-r from-[#0046FF] to-[#001BB7] text-white font-semibold rounded-lg shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-[#0046FF] focus:ring-offset-2 focus:ring-offset-transparent transition-all duration-300 transform hover:scale-[1.01] active:scale-[0.98]"
            >
              Öğrenci Ekle
            </button>
          </form>
              ...
              <table className="min-w-full divide-y divide-[var(--border-primary)]">
                <thead className="bg-transparent">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)]">Düzenle</th>
                  </tr>
                </thead>
                <tbody className="bg-transparent divide-y divide-[var(--border-primary)]">
                  {paginatedStudents.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-6 py-8 text-center text-[var(--text-tertiary)] text-base">Kayıtlı öğrenci bulunamadı.</td>
                    </tr>
                  ) : (
                    paginatedStudents.map(student => (
                      <tr key={student.id} className="hover:bg-[#f4f7fd]/40 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <button
                            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#0046FF]/10 text-[#0046FF] font-semibold text-xs hover:bg-[#0046FF]/20 transition-all border border-transparent hover:border-[#0046FF] focus:outline-none"
            ...
            {totalPages > 1 && (
              <div className="flex justify-center gap-2 mt-8">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                  <button
                    key={page}
                    className={`w-9 h-9 flex items-center justify-center rounded-full border text-base font-semibold transition-all duration-200 focus:outline-none ${currentPage === page ? 'bg-[#0046FF] text-white border-[#0046FF] scale-110 shadow' : 'bg-transparent text-[var(--text-secondary)] border-[var(--border-primary)] hover:bg-[#0046FF]/10 hover:text-[#0046FF]'}`}
      ...
      {editModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-2xl shadow-xl p-8 w-full max-w-md relative animate-fadeIn">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-[var(--border-primary)] bg-[var(--bg-tertiary)] focus:outline-none focus:ring-2 focus:ring-[#0046FF]/20 text-[var(--text-primary)]"
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>
            ...
            <div className="flex justify-end gap-2 mt-8">
              ...
              <button
                className="px-5 py-2 rounded-lg bg-gradient-to-r from-[#0046FF] to-[#001BB7] text-white font-semibold shadow hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-[#0046FF] focus:ring-offset-2 focus:ring-offset-transparent transition-all"
